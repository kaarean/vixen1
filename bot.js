...
let add = client.channels.get("688719387129020459")
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('Vixen  -|-  Reklam engellendi.', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("Vixen Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`VİXEN | Mod-Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`Vixen | Mod-Log Sistemi Kanal ID: ${channel.id}`)
				modlogkanal.send({embed});
			};
			if (channel.type === "voice") {
				let embed = new Discord.RichEmbed()
...
let bal  = db.fetch(`davet_${invite.inviter.id + member.guild.id}`)
   member.guild.channels.get(channel).send(`:inbox_tray: ** <@${member.id}> Katıldı**; Davet Eden **${davetçi.tag}** (`+'**'+bal+'** Daveti Var)')
  })
...
const ms = require('parse-ms')
  ...
  if (message.content.length > 1){ 
    db.set(`goldsure_${message.author.id}`, Date.now());
         ...
         .setColor('BLACK')
         .setTitle('Gold Üye!')
         .setDescription(' **Bir Gold Üye Belirdi!**')
